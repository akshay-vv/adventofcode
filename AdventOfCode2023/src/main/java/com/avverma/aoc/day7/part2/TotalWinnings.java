package com.avverma.aoc.day7.part2;
...
public class TotalWinnings {
    static String sCards = "AKQT98765432J";
    ...
    static class Hand implements Comparable<Hand>{
        ...
        private Type identifyType() {
            ...
            Map<Boolean, List<Character>> partition = hand.chars().mapToObj(i -> (char)i).collect(Collectors.partitioningBy(c -> c == 'J'));

            int nJokers = partition.get(true).size();

            partition.get(false).forEach(c -> count.merge(c, 1, Integer::sum));
            List<Integer> orderedCounts = count.values().stream().sorted().collect(Collectors.toList());

            // Increment the max count element by nJoker
            if (orderedCounts.isEmpty()) {
                orderedCounts.add(5);
            } else {
                int lastIdx = orderedCounts.size()-1;
                orderedCounts.set(lastIdx, orderedCounts.get(lastIdx) + nJokers );
            }
    ...
    static List<String> readInput() {
        try(BufferedReader br = new BufferedReader(new FileReader("src/main/java/com/avverma/aoc/day7/input.txt"))) {
