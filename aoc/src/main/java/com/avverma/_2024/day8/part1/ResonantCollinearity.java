package com.avverma._2024.day8.part1;
        ...
        for (Map.Entry<Character, List<int[]>> antenna : antennas.entrySet()) {
            ...
            for (int a = 0 ; a < loc.size(); a++) {
                for (int b = a+1; b < loc.size(); b++) {
                    ...
                    int[] antinodeC = new int[]{near[0] + near[0]-farther[0], near[1] + near[1]-farther[1] };
                    int[] antinodeD = new int[]{farther[0] + farther[0]-near[0], farther[1] + farther[1]-near[1] };


                    if (withinBounds(antinodeC)) {
                        antinodes.add(new Coordinate(antinodeC[0], antinodeC[1]));
                    }
                    if(withinBounds(antinodeD)) {
